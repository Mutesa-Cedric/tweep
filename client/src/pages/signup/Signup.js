...
const Signup=(props)=>{
    return (
        <div className={props.darkMode?"w-screen h-screen overflow-hidden flex items-center flex-col justify-center bg-[#252329]":"w-screen h-screen overflow-hidden flex items-center flex-col justify-center"}>
            <form method="post" onSubmit={props.submitForm} className="lg:w-[30%] md:w-[70%] sm:w-[100%] h-auto flex justify-center items-center rounded-3xl border-[1px] border-[#BDBDBD] px-14 py-8">
                <div className="w-full flex flex-col justify-center items-center">
                    ...
                    <div className="my-2">
                        <h1 className={props.darkMode?"font-[600] text-xl mb-3 text-[#E0E0E0] capitalize":"font-[600] text-xl mb-3 text-[#333333] capitalize"}>Join thousands of Tweepers from all over the world with one signup</h1>
                        <p className={props.darkMode?"text-[#E0E0E0]":""}>start live conversations with other people, tweep your experiences and have fun.</p>
                    </div>
                    <div className="my-2 flex flex-col w-full">
                        <input type="text" autoComplete="off" 
                        className={props.darkMode?"border-[1px] px-4 text-white border-[#BDBDBD] rounded-[8px] py-2 my-2 placeholder:text-lg bg-inherit":"border-[1px] border-[#BDBDBD] px-4 rounded-[8px] py-2 my-2 placeholder:text-lg "}
                        placeholder="Username"
                        name="userName"
                        value={props.formData.userName}
                        onChange={props.handleChange}
                        />
                        <p className="text-red-300 font-[600] mx-auto animate-pulse">{props.errorMessages.userNameMessage}</p>
                        <input type="Email" autoComplete="off" 
                        className={props.darkMode?"border-[1px] px-4 text-white border-[#BDBDBD] rounded-[8px] py-2 my-2 placeholder:text-lg  bg-inherit":"border-[1px] border-[#BDBDBD] rounded-[8px] py-2 my-2 placeholder:text-lg px-4 "}
                        placeholder="email"
                        name="email"
                        onChange={props.handleChange}
                        value={props.formData.email}
                        />
                        <p className="text-red-300 font-[600] mx-auto animate-pulse">{props.errorMessages.emailMessage}</p>
                        <input type="password" autoComplete="off" 
                         className={props.darkMode?"border-[1px] px-4 border-[#BDBDBD] rounded-[8px] py-2 my-2 placeholder:text-lg  text-white bg-inherit":"border-[1px] border-[#BDBDBD] rounded-[8px] py-2 my-2 placeholder:text-lg px-4 "}
                        placeholder="Password"
                        name="password"
                        onChange={props.handleChange}
                        value={props.formData.password}
                        />
                        <p className="text-red-300 font-[600] mx-auto animate-pulse">{props.errorMessages.passwordMessage}</p>
                    </div>
                    <p className="text-red-300 font-[600] mx-auto animate-pulse">{props.errorMessages.allFieldsMessage}</p>
